...
test_that("sampling_frame constructor works correctly", {
  ...
  sframe2 <- fmrihrf::sampling_frame(blocklens = c(100, 200), TR = c(2, 1.5))
  expect_equal(sframe2$TR, c(2, 1.5))
  
  # Test input validation
  expect_error(fmrihrf::sampling_frame(blocklens = c(-1, 100), TR = 2), 
              "Block lengths must be positive")
  expect_error(fmrihrf::sampling_frame(blocklens = c(100, 100), TR = -1), 
              "TR .* must be positive")
  expect_error(fmrihrf::sampling_frame(blocklens = c(100, 100), TR = 2, precision = 3),
...
test_that("global_onsets works correctly", {
  ...
  onsets <- c(10, 20)
  blockids <- c(1, 2)
  global_times <- fmrihrf::global_onsets(sframe, onsets, blockids)
...
test_that("sampling_frame handles edge cases", {
  # Single block
  single_block <- fmrihrf::sampling_frame(blocklens = 100, TR = 2)
  expect_equal(length(single_block$blocklens), 1)
  expect_equal(length(fmrihrf::samples(single_block)), 100)
  
  # Very short block
  short_block <- fmrihrf::sampling_frame(blocklens = c(1, 1), TR = 2)
  expect_equal(length(fmrihrf::samples(short_block)), 2)
