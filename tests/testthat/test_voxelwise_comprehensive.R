...
test_that("chunked voxelwise engine matches direct computation", {
  ...
  for (v in seq_len(V)) {
    ...
    Y[, v] <- X %*% rnorm(p) + noise
    tmp <- fmrireg:::ar_whiten_transform(X, Y[, v, drop = FALSE], phi[, v], exact_first = TRUE)
    qr_w <- qr(tmp$X)
  ...
  res_direct <- fit_lm_contrasts_voxelwise(Betas, sigma^2, XtXinv_list,
                                           clist, fclist, dfres)
  res_chunk <- fmrireg:::fit_lm_contrasts_voxelwise_chunked(X, Y, phi, clist, fclist,
                                                  chunk_size = 2)
...
test_that("constant voxel handled without error", {
  ...
  clist <- list(A = 1)  # Apply weight 1 to column 2
  attr(clist$A, "colind") <- 2L
  res <- fmrireg:::fit_lm_contrasts_voxelwise_chunked(X, Y, phi, clist, list(),
                                            chunk_size = 1)
...
test_that("effective df adjusts p-values for AR order", {
  ...
  clist <- list(A = 1)  # Apply weight 1 to column 2
  attr(clist$A, "colind") <- 2L
  res <- fmrireg:::fit_lm_contrasts_voxelwise_chunked(X, matrix(y, ncol = 1), phi,
                                            clist, list(), chunk_size = 1)
  tval <- res$A$data[[1]]$stat
  dfres <- n - qr(X)$rank
  df_eff <- fmrireg:::calculate_effective_df(n, ncol(X), ar_order = 1)
