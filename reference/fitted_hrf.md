...
Compute and return the fitted hemodynamic response function (HRF) for a
model object. The HRF represents the expected BOLD response to neural
activity. For models with multiple basis functions, this returns the
combined HRF shape.
...
A numeric vector containing the fitted HRF values at the requested time
points

## Details

This generic function computes the fitted hemodynamic response function
(HRF) for an object. The method needs to be implemented for specific
object types.
