...
run_subject <- function(sid) {
  ...
  ret <- fmrireg::fmri_lm(elab_onset ~ hrf(attention, accuracy, probe_type, basis=hrfb, subset=phase=="encoding" & felab!=0), #+ 
...
df2_wide <- df2 %>%
  tidyr::pivot_wider(names_from = row_id, values_from = beta, 
              id_cols = c(attention, basis, accuracy, probe_type, subject)) 

write.table(df2_wide, "~/Dropbox/analysis/SOD/fmri/glm_SOD_subsequent_betas_wide.txt", row.names=FALSE)
...
library(lme4)
library(lmerTest)
subsequent_by_basis <- lapply(1:404, function(i) {
  print(i)
  df2_roi<- dplyr::filter(df2, row_id==i & flag == "ok")
  
  lme1 <- lmer(beta ~ probe_type*accuracy + (1 | subject), data=subset(df2_roi, basis=="basis01"))
  lme2 <- lmer(beta ~ probe_type*accuracy + (1 | subject), data=subset(df2_roi, basis=="basis02"))
  
  lme1a <- lmer(beta ~ accuracy + (1 | subject), data=subset(df2_roi, basis=="basis01" & probe_type == "Recombined"))
  lme2a <- lmer(beta ~ accuracy + (1 | subject), data=subset(df2_roi, basis=="basis02" & probe_type == "Recombined"))
  ...
  #av2 <- anova(lme2,lme0)
  #pval2 <- av2$`Pr(>Chisq)`[2]
  data.frame(roi=i, t_acc_incorr_basis1=coef(summary(lme1))[3,4],
                    t_acc_incorr_basis2=coef(summary(lme2))[3,4],
                    t_acc_recom_incorr_basis1=coef(summary(lme1))[4,4],
                    t_acc_recom_incorr_basis2=coef(summary(lme2))[4,4],
                    t_acc_incorr_intact_basis1=coef(summary(lme1a))[2,4],
                    t_acc_incorr_intact_basis2=coef(summary(lme2a))[2,4])
             
}) %>% bind_rows()

write.table(subsequent_by_basis, "~/Dropbox/analysis/SOD/fmri/lmer_subsequent_by_basis.txt", row.names=FALSE)
