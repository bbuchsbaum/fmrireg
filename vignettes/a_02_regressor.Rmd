...
# Print the regressor object to see its properties
print(reg1)
...
# The plot.regressor method provides a quick base R plot
# plot(reg1, samples = scan_times)
```

The `plot()` method for regressor objects provides a quick way to visualize the evaluated timecourse, including vertical lines indicating the event onsets.
...
For some analyses (like beta-series correlation), you need to model *each individual trial* with its own regressor. The `single_trial_regressor()` function creates a regressor for just one event.

```{r single_trial}
# Create a regressor for a single trial at t=20s, duration 2s
st_reg <- single_trial_regressor(onsets = 20, hrf = HRF_SPMG1, duration = 2)
print(st_reg)

# Evaluate and plot
scan_times_st <- seq(0, 60, by = TR)
pred_st <- evaluate(st_reg, scan_times_st)

plot_df_st <- data.frame(Time = scan_times_st, Response = pred_st)
ggplot(plot_df_st, aes(x = Time, y = Response)) +
  geom_line(linewidth = 1) +
  geom_vline(xintercept = onsets(st_reg), linetype = "dashed", color = "red", alpha = 0.7) +
  labs(title = "Single Trial Regressor (Onset=20s, Duration=2s)",
       x = "Time (seconds)",
       y = "Predicted Response") +
  theme_minimal()
```

Typically, you would create a list of these single trial regressors, one for each event in your experiment.
