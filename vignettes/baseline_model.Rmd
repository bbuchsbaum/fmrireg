...
In addiiton to structured basis sets, we can also add arbitrary signals to the baseline model. Such singals are also organized by block on the assumption that nuisance signals are often block-specific and thus do not span across scans. To add a set of nusiance variables to the baseline model, then, they should be split by block and placed in a `list`.
