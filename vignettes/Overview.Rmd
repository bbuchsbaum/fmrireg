...
The `fmrireg` package provides tools for analyzing fMRI regression models in R, inspired by standard R modeling tools like `lm()` and `glm()`. It works in conjunction with the `fmridesign` package, which handles the design matrix construction aspects of fMRI analysis.

While fMRI analysis involves complexities like large datasets, spatio-temporal correlations, and physiological noise, the core of standard univariate analysis is multiple regression applied voxelwise. The `fmrireg` and `fmridesign` packages work together to simplify the specification and estimation of these regression models.
