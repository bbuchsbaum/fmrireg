...
The basic, or lowest level, unit of an fMRI regression model is the **hemodynamic response function** (HRF). The HRF is a model of the mapping, or transfer function, between an external stimulus (or neural event) and a BOLD signal. A commonly used HRF is the so-called SPM canonical HRF:
