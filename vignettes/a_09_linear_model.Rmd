...
path pools residual autocovariances across voxels, estimates one AR coefficient
vector for each run, whitens the data and design matrix, and refits the GLM.
This targets a typical voxel covariance. The experimental
`shared_estimator = "mean_series"` option instead averages voxel residuals at
each time point and targets the spatially coherent component, which can be much
more autocorrelated than an individual voxel. In either case, the matching OLS
design correction is valid: projection and cross-voxel averaging commute.
...
The `noise_spec()` constructor also accepts `struct = "arp"` for higher-order
autoregressive models. A positive `q` requests ARMA(p, q) inference on the
supported runwise-meta path; see `?noise_spec` for the current restrictions.
...
hrf_baseline <- baseline_model(
  "poly", degree = 1, sframe = fmridataset::as_sampling_frame(hrf_dataset)
)
```
...
extract_model_stats <- function(model, model_name, dataset) {
  # Get observed data
  observed_data <- fmridataset::collect_assay(dataset)
