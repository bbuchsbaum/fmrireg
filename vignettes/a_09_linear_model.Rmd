...
knitr::opts_chunk$set(fig.alt = "Figure illustrating model, design, or results; see caption and text for details.")
```
...
# Note: Robust fitting example - requires bug fix in chunkwise_lm_slow
...
beta_long <- as.data.frame(beta_estimates) %>%
...
t_stats <- stats(model)
std_errors <- standard_error(model)
```


```{r, message=FALSE, fig.width=7, fig.height=7, eval=FALSE}
# Note: Statistical results visualization - requires proper stats() method implementation
...
# The following code would combine and visualize statistical results:
...
# Note: Contrast analysis example - requires bug fix in event_model parsing
...
contrast_estimates <- coef(contrast_model)
contrast_tstats <- stats(contrast_model)
contrast_stderr <- standard_error(contrast_model)
...
```{r, message=FALSE, fig.width=7, fig.height=6, eval=FALSE}
...
```{r, message=FALSE, fig.width=7, fig.height=5, eval=FALSE}
# Note: HRF comparison example - requires fix in model fitting
...
```{r, message=FALSE, fig.width=7, fig.height=7, eval=FALSE}
# Note: AIC comparison visualization - requires model_comparison_long
...
- [Contrasts and Hypothesis Tests](/articles/a_05_contrasts.html)
