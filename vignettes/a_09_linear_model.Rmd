...
# Accessibility: provide default alt-text for figures
knitr::opts_chunk$set(collapse = TRUE, comment = "#>", fig.alt = "Figure illustrating model, design, or results; see caption and text for details.")
```
...
```{r create-event-table, message=FALSE, fig.width=7, fig.height=5}
...
```{r simulate-signals, message=FALSE, fig.width=7, fig.height=8}
...
fast AR(1) prewhitening step with `noise_spec("ar1")`. This estimates the
...
The AR(1) model now estimates a non-zero autoregressive coefficient and produces
notably smaller standard errors than the plain OLS fit, illustrating how
prewhitening improves efficiency when temporal autocorrelation is present.
...
Robust fitting guards against outlier time points but will not correct voxel-specific spikes. In this simulated example the Huber weights inflate the standard errors slightly, reflecting the additional uncertainty introduced by potential outliers. P-values rely on a robust residual scale and should be interpreted as approximate.
...
```{r plot-coefficients, message=FALSE, fig.width=7, fig.height=7}
...
```{r plot-contrast-results, message=FALSE, fig.width=7, fig.height=6}
...
```{r extract-fitted-hrf, message=FALSE, fig.width=7, fig.height=8}
...
```{r plot-r-squared, message=FALSE, fig.width=7, fig.height=5}
...
```{r plot-aic, message=FALSE, fig.width=7, fig.height=7}
