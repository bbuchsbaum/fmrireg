...
This vignette describes the main dataset classes and how to choose among them.
For a first complete fit, read `vignette("fmrireg", package = "fmrireg")`;
return here when you need to change how scans are stored or divided into runs.

The dataset classes and constructors are implemented by the companion
`fmridataset` package and re-exported by `fmrireg`, so the calls below are the
supported `fmrireg` user surface.
...
knitr::kable(data.frame(
  ...
  constructor = c(
    "fmri_mem_dataset()",
    "fmri_dataset() -> fmri_file_dataset",
    "matrix_dataset()",
    "latent_dataset()"
  ),
  choose_it_when = c(
    "The full volumes comfortably fit in memory",
    "Runs are large and should be opened lazily",
    "Data are ROI, surface, or already tabular",
    "A fitted fmristore representation already exists"
  )
), caption = "Choose a dataset representation from the storage you already have")
```
...
They provide a standardized way to access data (`get_data(dset)`) and masks
(`get_mask(dset)`). Timing belongs to the dataset's sampling frame, so inspect
it with `blocklens(dset$sampling_frame)` and
`blockids(dset$sampling_frame)`, regardless of the underlying storage format.
