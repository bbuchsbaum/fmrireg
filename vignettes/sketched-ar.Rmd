...
vignette: |
  %\VignetteIndexEntry{Sketched GLM with Global vs Parcel AR} %\VignetteEngine{knitr::rmarkdown} %\VignetteEncoding{UTF-8}
---
...
- Optional Nystrom spatial extension (solve on L landmarks, extend to all voxels)

At a glance
- Typical fidelity: correlations > .9 vs exact voxelwise GLM on synthetic and real designs, with large runtime savings.
- Knobs: m (sketch rows), iters (IHS iterations), by_cluster (parcel AR), landmarks (Nystrom), k_neighbors (Nystrom weights).
...
with `m = 8p` rows. Setting `by_cluster = FALSE` pools a single AR(1)
coefficient across the whole brain.
...
its solution over a few iterations; 2-3 iterations with `m ~ 6p` are
...
- SRHT/IHS preserve GLM geometry; Nystrom respects spatial structure.
