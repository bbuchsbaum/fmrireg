...
`fmri_mem_dataset` so it can be passed to `fmri_lm()`.
...
We fit the same model without sketching and correlate the resulting
beta maps. Correlations above 0.9 indicate that the sketch
faithfully preserves the GLM solution.
...
On this synthetic dataset you should see output like:
...
All three methods recover betas that correlate very highly with the
exact solution. The precise values will vary with the random seed
but are consistently above 0.9.
