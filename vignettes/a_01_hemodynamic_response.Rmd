...
## Modeling Event Duration with `hrf_blocked`

fMRI events often have a duration (e.g., a stimulus presented for several seconds). The `hrf_blocked` function (or `gen_hrf_blocked`) modifies an HRF to model the response to a sustained event of a specific `width` (duration). Internally, it convolves the original HRF with a boxcar function of the specified width.
...
hrf_spm_w1 <- hrf_blocked(HRF_SPMG1, width = 1)
hrf_spm_w2 <- hrf_blocked(HRF_SPMG1, width = 2)
hrf_spm_w4 <- hrf_blocked(HRF_SPMG1, width = 4)
...
By default, longer durations lead to higher peak responses (assuming summation, see next section). Setting `normalize=TRUE` in `hrf_blocked` rescales the response so the peak amplitude is approximately 1, regardless of duration.

```{r blocked_normalized}
# Create normalized blocked HRFs
hrf_spm_w1_norm <- hrf_blocked(HRF_SPMG1, width = 1, normalize = TRUE)
hrf_spm_w2_norm <- hrf_blocked(HRF_SPMG1, width = 2, normalize = TRUE)
hrf_spm_w4_norm <- hrf_blocked(HRF_SPMG1, width = 4, normalize = TRUE)
...
ggplot(plot_df_blocked_norm, aes(x = Time, y = Response, color = Duration)) +
  ...
  labs(title = "Normalized SPM Canonical HRF for Different Durations",
       subtitle = "Using hrf_blocked(normalize = TRUE)",
       x = "Time (seconds)",
       y = "BOLD Response",
       color = "Duration") +
  theme_minimal() +
...
The `summate` argument in `hrf_blocked` controls whether the response accumulates over the duration (`summate=TRUE`, default) or saturates (`summate=FALSE`). Saturation implies that the response reaches a plateau and doesn't increase further with longer stimulation.

```{r blocked_summate_false}
# Create non-summating blocked HRFs
hrf_spm_w2_nosum <- hrf_blocked(HRF_SPMG1, width = 2, summate = FALSE)
hrf_spm_w4_nosum <- hrf_blocked(HRF_SPMG1, width = 4, summate = FALSE)
hrf_spm_w8_nosum <- hrf_blocked(HRF_SPMG1, width = 8, summate = FALSE)
...
ggplot(plot_df_blocked_nosum, aes(x = Time, y = Response, color = Duration)) +
  ...
  labs(title = "Non-Summating (Saturating) SPM HRF for Different Durations",
       subtitle = "Using hrf_blocked(summate = FALSE)",
       x = "Time (seconds)",
       y = "BOLD Response",
       color = "Duration") +
  theme_minimal()
...
hrf_spm_w2_nosum_norm <- hrf_blocked(HRF_SPMG1, width = 2, summate = FALSE, normalize = TRUE)
hrf_spm_w4_nosum_norm <- hrf_blocked(HRF_SPMG1, width = 4, summate = FALSE, normalize = TRUE)
hrf_spm_w8_nosum_norm <- hrf_blocked(HRF_SPMG1, width = 8, summate = FALSE, normalize = TRUE)
...
ggplot(plot_df_blocked_nosum_norm, aes(x = Time, y = Response, color = Duration)) +
  ...
  labs(title = "Normalized, Non-Summating SPM HRF for Different Durations",
       subtitle = "Using hrf_blocked(summate = FALSE, normalize = TRUE)",
       x = "Time (seconds)",
       y = "BOLD Response",
       color = "Duration") +
  theme_minimal() +
...
We can combine `hrf_lagged` and `hrf_blocked` using the pipe operator (`%>%`) from `dplyr` (or `magrittr`).

```{r lagged_blocked_hrfs}
# Create HRFs that are both lagged and blocked
hrf_lb_1 <- HRF_GAUSSIAN %>% hrf_lagged(1) %>% hrf_blocked(width = 1, normalize = TRUE)
hrf_lb_3 <- HRF_GAUSSIAN %>% hrf_lagged(3) %>% hrf_blocked(width = 3, normalize = TRUE)
hrf_lb_5 <- HRF_GAUSSIAN %>% hrf_lagged(5) %>% hrf_blocked(width = 5, normalize = TRUE)
...
ggplot(plot_df_lagged_blocked, aes(x = Time, y = Response, color = Settings)) +
  ...
  labs(title = "Gaussian HRFs with Combined Lag and Duration",
       subtitle = "Using hrf_lagged() %>% hrf_blocked()",
       x = "Time (seconds)",
       y = "BOLD Response",
       color = "Settings") +
  theme_minimal() +
...
sim_profile <- rowMeans(replicate(20, {
  h <- hrf_spmg1 %>% hrf_lagged(lag = runif(n = 1, min = -1, max = 1)) %>%
                    hrf_blocked(width = runif(n = 1, min = 0, max = 2))
  h(sim_times)
}))
